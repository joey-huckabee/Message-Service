...
**Recommended sequence** (per 2026-04-27 audit synthesis): ~~29~~ → **30 → 31 → 33 → 34 → 32 → 24 → 28 → v1 tag.** 29 closed first per the audit ordering and set the `BackgroundTaskScheduler`-driven pattern for 30 to mirror. 31 / 33 / 34 are independent and could parallelize. 32 lands last among implementation increments so its matrix regens capture all upstream promotions in one final pass. 24 + 28 are documentation/examples; 24 ADRs benefit from every L1 being Implemented (or honestly deferred) so the document content doesn't go stale on commit.
