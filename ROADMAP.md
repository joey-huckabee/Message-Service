...
Last full increment merged: **32i — Trace-matrix coverage pass closeout** (this commit). Increment 32 closed across thirteen sub-steps (32a–32m + 32i) over 2026-04-28 and 2026-04-29. **Final v1 trace-matrix state: 62 of 67 L1 requirements Implemented; the remaining 5 are intentional v1 Partials each with a named R-XXX-NNN deferral entry in Part 2 — Deferred from v1.** 12 of the 16 categories are fully closed (RUN ✅, STAGE ✅, TMPL ✅, AUTH ✅, SUB ✅, MAIL ✅, SWEEP ✅, DEP ✅, CICD ✅, PERS ✅, CFG ✅; AGGR / API / DASH / OBS / ERR each have a single intentional Partial). Counts: **67 L1 / 192 L2 / 393 L3** across 16 categories; 446 of 585 reqs verified by at least one test (76.2%, up from 45.1% post-27). 1263 tests pass at 94.88% branch coverage (gate 85%). Six v1-cycle deferred-features captured so far: R-ERR-001, R-ERR-002, R-DASH-004 (the original 3) plus R-API-001 (gRPC infrastructure hardening), R-AGGR-001 (custom email body contributions from stages — pre-existing entry), and the L3 deferrals routing through them.
...
|---|---|---|
| L1-API-001 | L3-API-002/003/004 deferred (per-RPC interceptor + pinned-tag proto + CI version check) | R-API-001 |
...
- **Increment 32** *(✅ done — 13 sub-step commits ending at this commit)* — Trace-matrix coverage pass (the "Category C" cleanup). 41 of 44 Partial L1s + 4 of the 5 Draft L1s entered the increment; **48 L1 promotions across 32a-m** brought the matrix to its final v1 state. Sub-step commits: 32a `b81df1f` (API+RUN), 32b `6e70641` (STAGE+SWEEP+AGGR), 32c `304eadc` (SUB+AUTH), 32d `284c227` (MAIL+TMPL), 32e `873a0b5` (DASH+PERS+OBS+ERR+CFG initial — L1-PERS-003 + 6 L3 deferrals), 32f `6ac6095` (AGGR residuals), 32g `e59b224` (SWEEP residuals), 32h `c9a2d5d` (L2-RUN-016 L3 authoring), 32j `5eaf478` (DASH residuals), 32k `c471109` (PERS residuals), 32l `abb395b` (OBS residuals), 32m `cb73a94` (ERR+CFG residuals), 32i (this commit — closeout). Per-category close-out: every category except API/AGGR/DASH/OBS/ERR has 100% L1 Implemented; the five exceptions each carry one intentional Partial documented above. Across the increment: 393 L3 statements (up from 386 entering); 1263 tests pass at 94.88% branch coverage (gate 85%); trace matrix --check clean. AUTH session-memo correction landed (the 2026-04-26 "AUTH category fully closed" claim was premature — only L1-AUTH-003 was Implemented at that snapshot; 001/002 promoted in 32c).

**Still open (release-gating)**:

- **Increment 24** — Documentation deliverables. Two ADRs (SQLite-for-in-flight-state, hexagonal-boundary), operator runbook, pipeline integration guide, promote `tests/README.md` to formal test strategy. Increment 20d's remainder lives as deferred work in `R-DASH-004`. Safe to author now because every L1 is either Implemented or has an honest R-XXX-NNN deferral entry.
- **Increment 28** — Runnable demonstration examples. New top-level `examples/` directory with eight self-contained, fully-documented scenario scripts plus shared helpers. Each scenario has a complete README with verbatim expected output. Recommended slot: with or after 24 — they cross-reference each other.

**Recommended sequence**: ~~29~~ → ~~30~~ → ~~31~~ → ~~33~~ → ~~34~~ → ~~32~~ → **24 → 28 → v1 tag.** All implementation increments are landed. 24 + 28 are documentation/examples; 24 ADRs benefit from every L1 being Implemented (or honestly deferred) so the document content is stable on commit.
