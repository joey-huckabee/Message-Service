...
**Recommended sequence** (per 2026-04-27 audit synthesis): ~~29~~ → ~~30~~ → ~~31~~ → **33 → 34 → 32 → 24 → 28 → v1 tag.** 29 + 30 + 31 closed all three audit-identified drifts per the audit ordering — 29 set the `BackgroundTaskScheduler`-driven pattern; 30 mirrored it cleanly; 31 closed L1-MAIL-004 plus the residual spec-cleanup items (L2-OBS-007 verification method, L3-MAIL-001 reword). 33 / 34 are independent and could parallelize. 32 lands last among implementation increments so its matrix regens capture all upstream promotions in one final pass. 24 + 28 are documentation/examples; 24 ADRs benefit from every L1 being Implemented (or honestly deferred) so the document content doesn't go stale on commit.
