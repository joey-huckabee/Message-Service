...
- **Increment 32** — Trace-matrix coverage pass (the "Category C" cleanup). 41 of 44 Partial L1s + 4 of the 5 Draft L1s have code that already implements the requirement; the matrix shows them Partial only because L3 statements / test markers / small inspection tests are missing. **32a + 32b + 32c + 32d done** (`b81df1f`, `6e70641`, `304eadc`, `284c227`); **32e initial done** (`873a0b5`); 29 L1s promoted so far (L1-API-002/003/004 + L1-RUN-002/003/004 in 32a; all L1-STAGE-* + L1-SWEEP-002 + L1-AGGR-004 Draft → Partial in 32b; all L1-SUB-* + all L1-AUTH-* in 32c; all L1-MAIL-* + all L1-TMPL-* in 32d; L1-PERS-003 in 32e). Remaining sub-steps capture what 32a/b/e deliberately left open: **32f AGGR residuals** (L1-AGGR-001/002/003 from 32b), **32g SWEEP residuals** (L1-SWEEP-001/003 from 32b) + L1-API-001 documentation note, **32h L2-RUN-016 L3 authoring** (closes L1-RUN-005 from 32a), **32j DASH residuals** (L1-DASH-001/002/003 from 32e), **32k PERS residuals** (L1-PERS-001/002/004 from 32e), **32l OBS residuals** (L1-OBS-001/003/004 from 32e), **32m ERR + CFG residuals** (L1-ERR-001/003/004 + L1-CFG-001/002 from 32e), 32i matrix regen + AUTH-session-memo correction + done. Promotes ~41 + 4 L1s to Implemented across the increment, with the explicit exceptions of L1-API-001 (R-API-001 deferral), L1-DASH-004 (R-DASH-004 deferral), and L1-ERR-002 (R-ERR-002 deferral) that stay Partial by design.
...
**32i — Final matrix regen + session-memo correction + ✅ done** *(closeout; renumbered from 32f)*. Run `poetry run python scripts/build-trace-matrix.py` after 32c/d/e/f/g/h/j/k/l/m all complete. Verify `--check` is clean. Correct the 2026-04-26 session memo statement claiming "AUTH category fully closed: L1-AUTH-001/002/003 all Implemented" — only L1-AUTH-003 was Implemented at that time; L1-AUTH-001/002 promoted to Implemented in 32c. Status snapshot final regen with all promoted L1s rolled up. Banner refresh. Verify the residuals captured in 32f/g/h/j/k/l/m all closed; document any remaining intentional Partials (L1-API-001 per R-API-001; L1-DASH-004 per R-DASH-004; L1-ERR-002 per R-ERR-002; any others discovered en route) so the v1 release can ship with an honest matrix.
