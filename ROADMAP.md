...
Last full increment merged: **28 — Runnable demonstration examples** (this commit). Increment 28 ships a new top-level `examples/` directory with eight self-contained, fully-documented scenario scripts (`01-hello-world` through `08-error-recovery`), shared helpers under `examples/_lib/` (in-process `aiosmtpd` SMTP capture, service-subprocess runner, stdlib-only colored output, expectation DSL, common config/template helpers), and a conformance test (`tests/conformance/test_examples_present.py`) verifying the layout. Every scenario boots `python -m message_service` as a subprocess against a scenario-local config, drives the gRPC API (and the dashboard's `POST /runs/{run_id}/resend` for scenario 07), captures emails via the in-process SMTP mock, asserts ✓/✗ expectations, and exits 0/1 — making each scenario both a hand-runnable demo and a smoke test. Prior: **24 — Documentation deliverables**. **All v1 work is landed; the next commit tags v1.** Trace state unchanged from 32: 67 L1 / 192 L2 / 393 L3; 62 of 67 L1s Implemented (5 intentional v1 Partials each with a named R-XXX-NNN deferral entry in Part 2). 1263 tests pass at 94.88% branch coverage (gate 85%); the new conformance test adds 106 inspection assertions over the examples layout (no execution in CI — see Increment 28 sub-step 6). Six v1-cycle deferred-features captured: R-ERR-001, R-ERR-002, R-DASH-004 (original 3) plus R-API-001 (gRPC infrastructure hardening), R-AGGR-001 (custom email body contributions, pre-existing).
