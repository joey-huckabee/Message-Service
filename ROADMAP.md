...
Last full increment merged: **34 — L1-TMPL-004 JSON Schema context validation** (this commit); promotes L1-TMPL-004 Draft → Implemented and closes the last L1-promoting implementation increment before the trace-matrix coverage pass. Renderer constructor now eagerly compiles a `jsonschema.Draft202012Validator` per template entry whose manifest declares a `context_schema_path` (failing schemas raise `ConfigurationError` at service start, never at render time, per L3-TMPL-018 / L3-TMPL-031); render-time validation runs after the size pre-check and before Jinja2 invocation, surfacing violations as `ContextSchemaViolationError` with a slash-prefixed JSON Pointer (`exc.absolute_path` rendered per RFC 6901, escapes for `/` and `~`) plus `validator`, `instance_value`, and `message` fields per L3-TMPL-020 / L3-TMPL-032. Templates without `context_schema_path` skip validation entirely (L3-TMPL-030). Four new L3 statements (L3-TMPL-029..032) and one refined L3 (TMPL-018 now explicit about eager-at-load); 16 new unit tests covering construction-time meta-schema rejection, render-time validation, no-schema bypass, internal `$defs` reference resolution, RFC 6901 pointer encoding, and the size-before-schema ordering invariant; 2 new gRPC translator tests pinning CONTEXT_SCHEMA_VIOLATION → INVALID_ARGUMENT with the error code in trailing metadata. Prior: **33 — L1-TMPL-002 "latest" version resolution** (`9880971`, `a141e02`, `5596ea6`, `33345cb`). Earlier: **31 — L1-MAIL-004 admin notification** (`94864ef`+); **30 — Audit-log retention pruner** (`42455b4`+); **29 — Rendered-report retention pruner** (`fb3ab75`+); **27 — UoW serialization correctness** (`410aa90`+); **26e — L1-CICD trace-matrix closure** (commit `af59b43`). **Three deferred-features captured this v1 cycle**: R-ERR-001 (gRPC error envelope upgrade), R-ERR-002 (error-code lockfile), R-DASH-004 (embedded Chart.js dashboard).
...
A 2026-04-27 audit (post-27) categorized the remaining v1 closure work. Three implementation drifts (similar shape to the connection-pool drift Increment 27 fixed; **29 + 30 + 31 closed all three**), one feature gap discovered while categorizing the 5 Draft L1s (closed by **34**), one architectural-pattern increment for L1-TMPL-002 (closed by **33**), and a large but mechanical trace-matrix coverage pass for 41 of 44 Partial L1s where code already exists. Increments 24 + 28 (previously thought to be the last work) are now the *final* gating steps after the new increments land.
- **Increment 32** — Trace-matrix coverage pass (the "Category C" cleanup). 41 of 44 Partial L1s + 4 of the 5 Draft L1s have code that already implements the requirement; the matrix shows them Partial only because L3 statements / test markers / small inspection tests are missing. **32a + 32b done** (`b81df1f`, `6e70641`); 11 L1s promoted (L1-API-002/003/004 + L1-RUN-002/003/004 in 32a; all L1-STAGE-* + L1-SWEEP-002 in 32b; L1-AGGR-004 Draft → Partial in 32b). Remaining sub-steps follow the original audit chunk structure plus three new residual sub-steps capturing what 32a/b deliberately left open: 32c SUB+AUTH, 32d MAIL+TMPL, 32e DASH+PERS+OBS+ERR+CFG, **32f AGGR residuals** (L1-AGGR-001/002/003 from 32b), **32g SWEEP residuals** (L1-SWEEP-001/003 from 32b) + L1-API-001 documentation note, **32h L2-RUN-016 L3 authoring** (closes L1-RUN-005 from 32a), 32i matrix regen + AUTH-session-memo correction + done. Promotes ~41 + 4 L1s to Implemented across the increment, with the explicit exceptions of L1-API-001 (R-API-001 deferral) and L1-ERR-002 (R-ERR-002 deferral) that stay Partial by design.
...
**Recommended sequence** (per 2026-04-27 audit synthesis): ~~29~~ → ~~30~~ → ~~31~~ → ~~33~~ → ~~34~~ → **32 → 24 → 28 → v1 tag.** 29 + 30 + 31 closed all three audit-identified drifts; 33 closed the L1-TMPL-002 "latest" sentinel drift; 34 closed the L1-TMPL-004 JSON Schema validation drift (the last L1-promoting implementation increment). 32 lands next so its matrix regens capture all upstream promotions in one final pass. 24 + 28 are documentation/examples; 24 ADRs benefit from every L1 being Implemented (or honestly deferred) so the document content doesn't go stale on commit.
...
**32i — Final matrix regen + session-memo correction + ✅ done** *(closeout; renumbered from 32f)*. Run `poetry run python scripts/build-trace-matrix.py` after 32c/d/e/f/g/h all complete. Verify `--check` is clean. Correct the 2026-04-26 session memo statement claiming "AUTH category fully closed: L1-AUTH-001/002/003 all Implemented" — only L1-AUTH-003 was Implemented at that time; L1-AUTH-001/002 promote to Implemented in 32c. Status snapshot final regen with all promoted L1s rolled up. Banner refresh. Verify the residuals captured in 32f/32g/32h all closed; document any remaining intentional Partials (L1-API-001 per R-API-001; L1-ERR-002 per R-ERR-002; any others discovered en route) so the v1 release can ship with an honest matrix.

**Trace impact** (cumulative across 32a–32h): ~41 Partial L1s + 4 Draft L1s promote to Implemented, with the explicit exceptions of L1-API-001 (R-API-001 deferral) and L1-ERR-002 (R-ERR-002 deferral). Per-category L2-Implemented and L3-Implemented counts increase substantially. Overall verified-by-test percentage moves from 45.1% (post-27) and 51.9% (post-32a-b) toward 80%+.

**Sequencing**: Land **last** among the implementation increments (after 29, 30, 31, 33, 34) so its matrix regenerations capture all the upstream promotions in one final pass. Sub-steps 32a–32e are the audit-chunk-style coverage passes; 32f/g/h close the residuals surfaced during 32a-b; 32i is the closeout.
...
- **R-API-001 — gRPC infrastructure hardening: per-RPC correlation interceptor + pinned-tag proto dependency + CI version-mismatch check** — three related v1 carve-outs covered by this single deferral entry. (1) **Per-RPC structlog interceptor** (originally `L3-API-002`): v1 binds `correlation_id` only on the unexpected-error path (per `L3-ERR-017` / `L3-API-014`). The richer shape — a server-side interceptor that binds a fresh correlation id to the structlog context at every RPC entry (success and failure) and clears it in a `finally` block — is useful when the service participates in a larger distributed tracing context. Likely paired with the `R-OBS-001` distributed-tracing promotion. (2) **Pinned-tag proto dependency** (originally `L3-API-003`): v1's `pyproject.toml` declares `message-service-proto` as a local-path develop dependency (`{ path = "../message-service-proto", develop = true }`) because the proto package has not yet been published with a release cadence. Switching to a `{ git = ..., rev = "vX.Y.Z" }` form (or a registry pin) requires the proto repo first ship release tags. (3) **CI proto-version-mismatch check** (originally `L3-API-004`): a CI step asserting `message_service_proto.__version__` matches the Poetry-resolved version, gating the build on disagreement. Tautologically passes today (local-path develop resolves the same source the lockfile points at) and becomes meaningful only once the dependency is tag-pinned. Future work, sequenced together: (a) the proto repo publishes release tags; (b) `pyproject.toml` switches to a tag-pinned dependency with `develop = true` confined to local development overlays; (c) CI gains a `python -c "import message_service_proto, sys; sys.exit(0 if message_service_proto.__version__ == ...` check; (d) the per-RPC interceptor lands alongside or after the distributed-tracing infrastructure (R-OBS-001). All three trigger off the same operational condition: the gRPC ingress crosses a stable trust boundary with external consumers depending on a versioned wire contract — same trigger as the mTLS / RBAC items.
