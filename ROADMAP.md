...
**Recommended sequence** (per 2026-04-27 audit synthesis): ~~29~~ → ~~30~~ → ~~31~~ → ~~33~~ → **34 → 32 → 24 → 28 → v1 tag.** 29 + 30 + 31 closed all three audit-identified drifts; 33 closed the L1-TMPL-002 "latest" sentinel drift. 34 is the last L1-promoting implementation increment (L1-TMPL-004 JSON Schema validation). 32 lands last among implementation increments so its matrix regens capture all upstream promotions in one final pass. 24 + 28 are documentation/examples; 24 ADRs benefit from every L1 being Implemented (or honestly deferred) so the document content doesn't go stale on commit.
