...
**L3-API-002** · Parent: L2-API-001 · Verification: I, T
The gRPC server SHALL be constructed with a server interceptor (`grpc.aio.ServerInterceptor`) that, at the entry of every RPC (success and failure alike), binds a fresh `correlation_id` (a `uuid4().hex`) into the structlog contextvars via `bind_request_context`, and clears the request context via `clear_request_context` in a `finally` so it cannot leak to a subsequent RPC handled on the same worker task. The unexpected-error translator (`L3-ERR-017` / `L3-API-014`) SHALL reuse the interceptor-bound `correlation_id` for its `x-message-service-correlation-id` trailing metadata when one is bound, minting a fresh id only when none is bound (e.g. a unit test calling the translator directly), so a failed RPC surfaces to the client the same id its server-side log records carry. A test SHALL assert the id is bound during a successful RPC, that it is the same id surfaced on a failed RPC, and that the context is cleared after each call.
...
**L3-API-004** · Parent: L2-API-002 · Verification: I, T
A CI step, implemented as `scripts/check-proto-version.py`, SHALL assert that the installed `message_service_proto.__version__` equals the tag pinned for the `message-service-proto` dependency in `pyproject.toml` (`tag = "vX.Y.Z"`, with the leading `v` stripped), aborting the build on disagreement so a lockfile that resolves a different proto version than the manifest pins is caught at PR time. The script SHALL exit `0` on match, `1` on mismatch (naming both versions), and `2` when it cannot determine either version (missing dependency, tag, or `__version__`). A test SHALL assert the match, mismatch, and undeterminable outcomes via the script's importable comparison helper.
...
**L3-AUTH-018** · Parent: L2-AUTH-010 · Verification: T
The config schema SHALL carry an optional `[auth.admin]` section modeled as `AdminAccountConfig` with `email: str` and `password: SubstitutableStr` (both required when the section is present; environment substitution applies to `password` exactly as it does for `smtp.password`). The `[auth.admin]` section SHALL default to absent (`auth.admin is None`), in which case no admin is provisioned. When present, an empty `password` after substitution SHALL be rejected at load time. Tests SHALL cover: section absent → `None`; section present with valid values → parsed; empty post-substitution password → validation error.
...
**L3-MAIL-034** · Parent: L2-MAIL-014 · Verification: T
The subject of every outbound pipeline-report email SHALL be produced by a single shared construction — `AssembleAndDeliverUseCase.build_subject(run)`, which applies the `L3-MAIL-027` default format, the `L3-MAIL-032` per-pipeline `subject_templates` override, and the `L3-MAIL-028` sanitization identically. Both the first-delivery path (`AssembleAndDeliverUseCase.execute`) and the manual-resend path (`ResendRunUseCase`, which already reuses `AssembleAndDeliverUseCase.prepare_email` for the body) SHALL obtain the subject from this method rather than composing their own; the resend path SHALL NOT use the previously-hardcoded `Run {run_id} -- {pipeline_type}` form. A test SHALL assert that a resend of a pipeline with a configured `subject_templates` override carries the overridden subject, and that a resend of an unconfigured pipeline carries the `L3-MAIL-027` default (not the old resend-only format).
...
The dashboard ships hand-authored HTML pages (login, admin console, metrics dashboard, run-status board, subscriptions console). Their CSS and JS are packaged as static assets under `src/message_service/interfaces/rest/static/` and read at render time via `importlib.resources` and **inlined** into each self-contained page as `<style>…</style>` / `<script>…</script>` (there is no FastAPI `StaticFiles` mount and no `/static` URL — every page is a single self-contained document). Every asset SHALL be authored in-repo with NO third-party UI library, charting library, CDN reference, or other network-fetched resource — the metrics dashboard in particular renders its charts as hand-authored inline SVG, not via any external charting library. The L2-DASH-003 air-gap constraint is therefore actively satisfied: a conformance test (`L3-DASH-017`/`L3-DASH-039`) text-scans the shipped assets and asserts no external origin is referenced.

**L3-DASH-006** · Parent: L2-DASH-003 · Verification: A
v1 ships no Jinja2 HTML templates for dashboard rendering (only the email-body / per-stage-fragment / admin-notification templates which are operator-controlled and not user-facing). Dashboard HTML is produced by pure Python render functions that inline packaged CSS/JS assets per `L3-DASH-005`; the no-external-origin guarantee for those shipped assets is enforced by the `L3-DASH-017`/`L3-DASH-039` static scan.
...
The dashboard page SHALL render the embedded metric model as inline SVG using hand-authored, packaged static CSS/JS assets — no third-party charting library, no CDN, no network-fetched resource of any kind. A conformance test SHALL scan the dashboard's shipped assets (the static CSS/JS) and assert they reference no external origin: no `http`/`https` URL other than the standard W3C XML/SVG namespace URIs (`http://www.w3.org/...`, which are namespace identifiers required by `createElementNS` and are never fetched), no protocol-relative `//` resource reference, and none of the known CDN / charting-library markers (e.g. `cdn`, `unpkg`, `jsdelivr`, `googleapis`, `chart.js`, `d3`, `plotly`). This enforces the offline/no-dependency guarantee mechanically rather than by review. The visual correctness of the rendering is verified by Demonstration under `L1-DASH-004`.
...
**L3-DASH-037** · Parent: L2-DASH-017 · Verification: T
The service SHALL expose `GET /runs/board` behind the `require_session` dependency, declared in the runs router *before* the `GET /runs/{run_id}` route so the literal path is not captured as a `run_id`. The handler SHALL, server-side, obtain run summaries via `list_past_runs` across the full `RunState` set (so in-flight and terminal runs both appear), project each to the `L2-DASH-013` metadata field set, and return a self-contained HTML document embedding that projection as JSON (with `</` neutralized so the payload cannot close the embedding `<script>`). A test SHALL assert the route is session-gated (401/403 without a session), that an authenticated user receives an HTML document embedding the run projection, that `run_id="board"` resolves to the board rather than the detail route, and that the projection carries the same fields as the JSON summary.

**L3-DASH-038** · Parent: L2-DASH-018 · Verification: I
The board page SHALL render the embedded run projection using hand-authored, packaged static CSS/JS assets: a per-state summary distinguishing in-flight (`INITIATED`/`AGGREGATING`/`READY`/`SENDING`) from terminal (`SENT`/`FAILED`/`ORPHANED`) runs, a state filter, and a run row that expands to show its stages — the stage list fetched lazily from `GET /runs/{run_id}` (`L3-DASH-026`). No third-party library, no CDN, no network-fetched resource other than the same-origin runs API. The visual correctness of the rendering is verified by Demonstration under `L1-DASH-006`.

**L3-DASH-039** · Parent: L2-DASH-018 · Verification: I
A conformance test SHALL scan the board's shipped static assets (its CSS/JS) and assert they reference no external origin: no `http`/`https` URL other than the standard W3C XML/SVG namespace URIs, no protocol-relative `//` resource reference, and none of the known CDN / library markers (e.g. `cdn`, `unpkg`, `jsdelivr`, `googleapis`, `chart.js`, `d3`, `plotly`). This reuses and extends the `L3-DASH-017` mechanism so the offline/no-dependency guarantee is enforced across every shipped dashboard asset, not just the metrics ones. The scan SHALL cover **every** shipped static asset (metrics, board, login, console, and any future page), guarded by a test that fails if a static asset is added without being added to the scan.

**L3-DASH-040** · Parent: L2-DASH-019 · Verification: T
The service SHALL expose `GET /login` with no auth dependency, returning a self-contained HTML page (inlined packaged CSS/JS, no external reference) rendered by a pure `render_login_page()` function. The page SHALL contain an email field, a password field, and a submit control; its client code SHALL `POST` a JSON `{email, password}` body to the existing `POST /login`, then set `window.location` to `/admin/console` on `200` and show a generic error on `401`, without altering the `POST /login` endpoint. Tests SHALL assert `GET /login` returns HTML containing the form to an unauthenticated caller, and (inspection) that the shipped login JS posts to `/login` and redirects to `/admin/console`; the no-external-reference guarantee for the login assets is covered by `L3-DASH-039`.

**L3-DASH-041** · Parent: L2-DASH-020 · Verification: T
The service SHALL expose `GET /admin/console` behind the `require_admin` dependency, returning a self-contained HTML console rendered by a pure `render_admin_console()` function (inlined packaged CSS/JS, no external reference). The client SHALL fetch the roster from `GET /admin/users`, render each recipient (email, display name, role, status), and drive create / update / disable / reset-password via the existing `POST /admin/users`, `PATCH /admin/users/{id}`, `POST /admin/users/{id}/password` routes, echoing the `msp_csrf` cookie as `X-CSRF-Token`; a `401` from any call SHALL redirect to `GET /login`. Tests SHALL assert the route is admin-gated (401 unauth, 403 non-admin) and returns HTML, plus an inspection test that the shipped console JS reads `/admin/users`, sends the CSRF header, and references the three write routes.

**L3-DASH-042** · Parent: L2-DASH-021 · Verification: T
`UserRepository` SHALL gain a `list_paginated(limit, offset)` method returning `User` aggregates ordered deterministically by `user_id` ascending, implemented as raw SQL over the `users` table (reusing the existing row→`User` mapping). The no-`password_hash`-on-the-wire guarantee lives in the list endpoint's response projection (`L3-DASH-043`), not the repository. Tests SHALL cover ordering and the limit/offset slice.

**L3-DASH-043** · Parent: L2-DASH-021 · Verification: T
`GET /admin/users` SHALL be admin-gated, accept `limit` (`[1, 200]`, default 50) and `offset` (`>= 0`, default 0) with FastAPI returning `422` for out-of-range values, and return a JSON list of items conforming to a pydantic model with `extra="forbid"` exposing exactly `user_id, email, display_name, is_admin, disabled, created_at` (ISO-Z string). Tests SHALL assert the auth gate (401/403), the projected field set (no `password_hash`), and pagination behavior.

**L3-DASH-044** · Parent: L2-DASH-022 · Verification: T
The service SHALL provide `AdminSubscribeUseCase` and `AdminUnsubscribeUseCase`. `AdminSubscribeUseCase.execute(admin_id, target_user_id, granularity, target_value)` SHALL verify the target user exists (`UserNotFoundError` → 404), validate `PIPELINE`/`TAG` targets against the registered pipelines / tag vocabulary (reusing the same rule as `SubscribeUseCase`; unknown → `UnknownPipelineTypeError`/`UnknownTagError` → 422), add the subscription for `target_user_id`, and audit `SUBSCRIBE` with `actor=user:{admin_id}`, `resource=user:{target_user_id}`, and `details` carrying `{target_user_id, granularity, target_value}`. `AdminUnsubscribeUseCase.execute(admin_id, target_user_id, subscription_id)` SHALL delete the subscription only if it belongs to `target_user_id` (else `SubscriptionNotFoundError` → 404), and audit `UNSUBSCRIBE` with the same actor/resource shape plus `{subscription_id}`. Both commit the domain change and the audit row in one UoW. Tests SHALL cover create (incl. unknown user / invalid target / duplicate), delete (incl. id-not-owned-by-target → 404), and the actor/resource audit fields.

**L3-DASH-045** · Parent: L2-DASH-022 · Verification: T
The service SHALL expose, admin-gated, `GET /admin/users/{user_id}/subscriptions` (JSON list of the target's subscriptions — `subscription_id, granularity, target_value` each, `extra="forbid"`), `POST /admin/users/{user_id}/subscriptions` (body `{granularity, target_value}`, 201 on success, 404 unknown user, 422 invalid target, 409 duplicate), and `DELETE /admin/users/{user_id}/subscriptions/{subscription_id}` (204 on success, 404 when the id is not a subscription of the target user). Tests SHALL assert the auth gate (401/403) on each and the documented status codes.
...
The gRPC per-RPC correlation interceptor of `L3-API-002` SHALL cause every structlog record emitted while handling an RPC — on the success path, not only the unexpected-error path — to carry the request's `correlation_id` field automatically (via `structlog.contextvars.merge_contextvars`, already in the processor chain). A test SHALL drive a successful RPC through the interceptor and assert a captured log record carries a `correlation_id`.

**L3-OBS-004** · Parent: L2-OBS-002 · Verification: T
The FastAPI app SHALL install a per-request middleware that binds a fresh `correlation_id` (a `uuid4().hex`) into the structlog contextvars at request entry via `bind_request_context` and clears the request context via `clear_request_context` in a `finally`, so every log record emitted while handling a dashboard request carries the id and no id leaks between requests. The middleware SHALL be registered so it runs outermost (bound before authentication/CSRF logging). A test SHALL assert a request-scoped log record carries a `correlation_id` and that the context is cleared afterward.
...
**L3-OBS-041** · Parent: L2-OBS-019 · Verification: T
The configuration schema SHALL expose `observability.audit.archive_directory` as an optional filesystem path, defaulting to unset (no archival). When set, the composition root SHALL create the directory if missing and verify it is writable (create-and-probe), raising `ConfigurationError` before any use case is constructed if creation or the probe write fails — the same discipline `L3-PERS-011` applies to the rendered-report directory. A test SHALL assert the default-unset behavior and the writable-probe failure path.

**L3-OBS-042** · Parent: L2-OBS-019 · Verification: T
...
`except BaseException` is permitted ONLY at the gRPC servicer's translator chokepoint (`interfaces/grpc/servicer.py`), where the explicit `translate_to_grpc_status(context, exc)` call routes ordinary exceptions through the boundary translator while re-raising control-flow `BaseException` subclasses (`KeyboardInterrupt`, `SystemExit`, `asyncio.CancelledError`) unchanged per `L3-ERR-022`. Outside the servicer, `except BaseException`, `except SystemExit`, `except KeyboardInterrupt`, and `except GeneratorExit` SHALL NOT appear; domain and application code SHALL catch only `MessageServiceError` subclasses or specific stdlib exceptions. A conformance test enforces the chokepoint discipline.

**L3-ERR-022** · Parent: L2-ERR-010 · Verification: T
A unit test SHALL deliberately pass control-flow `BaseException` subclasses (`KeyboardInterrupt`, `SystemExit`, `asyncio.CancelledError`) through the translation layer and assert they propagate without emitting a gRPC abort.
...
| 2026-07-19 | Joey   | Run-status board: added L3-DASH-037 (`GET /runs/board` route + server-side summary projection, session-gated, path ordered before `/{run_id}`), L3-DASH-038 (hand-authored rendering: state summary + filter + lazy stage drill-in), L3-DASH-039 (no-external-ref conformance extended to the board assets) under new L2-DASH-017/018. Total: 414. |
| 2026-07-19 | Joey   | Admin console + login (v0.15.0): added L3-AUTH-018 (`[auth.admin]` config) / L3-AUTH-019 (startup reconciliation) under new L2-AUTH-010/011; L3-DASH-040 (`GET /login` page + submit flow), L3-DASH-041 (`GET /admin/console` over the admin account APIs + CSRF), L3-DASH-042 (`UserRepository.list_paginated`), L3-DASH-043 (`GET /admin/users`) under new L2-DASH-019/020/021; reworded L3-DASH-039 to scan every shipped static asset. Total: 420. |
| 2026-07-19 | Joey   | Admin subscription management (v0.16.0): added L3-DASH-044 (`AdminSubscribe`/`AdminUnsubscribe` use cases: target validation + actor=admin audit), L3-DASH-045 (on-behalf-of `/admin/users/{id}/subscriptions` routes + status codes), L3-DASH-046 (`GET /admin/subscriptions` console with embedded vocab dropdowns) under new L2-DASH-022/023. Total: 423. |
