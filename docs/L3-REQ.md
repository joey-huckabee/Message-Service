...
`UserRepository` SHALL gain a `list_paginated(limit, offset)` method returning accounts ordered deterministically by `user_id` ascending, implemented as raw SQL selecting `user_id, email, display_name, is_admin, disabled, created_at` (never `password_hash`). Tests SHALL cover ordering, the limit/offset slice, and that the hash column is not returned.
